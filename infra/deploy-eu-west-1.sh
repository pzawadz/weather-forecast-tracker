#!/bin/bash
#
# Deploy Weather Dashboard - EU-WEST-1 (Ireland)
# Ireland server already has weather tracker running
#
...
REGION="eu-west-1"
INSTANCE_ID="i-0acff8ad77effe38c"  # Ireland app server
...
echo "🔍 Finding subnets in VPC $VPC_ID (Ireland)..."
...
echo "🔐 Creating secret in AWS Secrets Manager (eu-west-1)..."
aws secretsmanager create-secret \
    --name weather-dashboard-cf-secret-eu \
    --description "CloudFront validation secret for Weather Dashboard (Ireland)" \
...
# Deploy stack
echo "🚀 Deploying CloudFormation stack (EU-WEST-1)..."
...
# Modify template to use eu-west-1 secret name
sed -i.bak 's/weather-dashboard-cf-secret/weather-dashboard-cf-secret-eu/g' cloudformation-dashboard.yaml
...
echo ""
echo "Run this command to allow ALB → EC2:8502:"
echo ""
...
echo "  2. Add inbound rule: TCP 8502 from $ALB_SG_ID"
echo ""
echo "Note: Dashboard runs on port 8502 (different from us-east-1)"
echo ""
